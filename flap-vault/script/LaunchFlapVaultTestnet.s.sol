...
import {VanityHelper} from "../test/lib/VanityHelper.sol";
...
/// @title LaunchFlapVaultTestnet — TRUE Flap vault-backed launch on BSC TESTNET (chainId 97)
///
...
///           1. Run DeployFlapFactoryTestnet (broadcast) → note the logged factory + 3 mock stock addrs.
///           2. Export them and run this script:
///                export FACTORY=0x…  STOCK0=0x…  STOCK1=0x…  STOCK2=0x…
///           If FACTORY is unset, this script BOOTSTRAPS a throwaway factory + mocks inline (so the DRY-RUN
...
///           forge script script/LaunchFlapVaultTestnet.s.sol --tc LaunchFlapVaultTestnet \
///             --rpc-url https://data-seed-prebsc-1-s1.bnbchain.org:8545
///         Add --broadcast --legacy to actually send.
contract LaunchFlapVaultTestnet is Script, VanityHelper {
    ...
    function _resolveDeployment() internal {
        ...
        // Bootstrap: no FACTORY env → deploy a throwaway factory + mocks so the dry-run runs end-to-end.
        bootstrapped = true;
    ...
    // ── VaultDataV1 (fees 2500/100/2500, weights 3400/3300/3300 → sum 10000) ────

    function _vaultData(address treasury) internal view returns (bytes memory) {
        address[] memory st = new address[](3);
        st[0] = stocks[0];
        st[1] = stocks[1];
        st[2] = stocks[2];
    ...
    function _report(address token, address vault, uint256 setupGas, uint256 fund) internal view {
        ...
        console2.log("== LaunchFlapVaultTestnet (chainId 97) ==");
